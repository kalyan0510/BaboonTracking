...
def non_max_suppression(im, window_shape=None, val=1.0):
    im = im.copy()
    ...
    for i in range(w_h, im.shape[0] - w_h):
        for j in range(w_l, im.shape[1] - w_l):
            t = im[i, j]
            im[i, j] = np.NINF
            op[i, j] = ([val, t][val is None]) if t > im[i - w_h:i + w_h + 1, j - w_l:j + w_l + 1].max() else 0.0
            im[i, j] = t
    return op
...
def  get_png_files(base_directory):
    file_list = os.listdir(base_directory)
    file_list = list(filter(lambda a: a.endswith('.png'), file_list))
    file_list = [file_path for file_path in file_list]
    return file_list


def load_png(paths, channels=[2, 1, 0]):
    return [imread(path, channels) for path in paths]
